...
function Movies() {
    ...
    function onLanguageFilterItemClicked(index: number) {
        ...
        updatedLanguages[index] = { ...updatedLanguages[index], filterApplied: !updatedLanguages[index].filterApplied };
    ...
    function onGenreFilterItemClicked(index: number) {
        ...
        updatedGenres[index] = { ...updatedGenres[index], filterApplied: !updatedGenres[index].filterApplied };
                            ...
                            <div className=''>
                                <ul className='flex flex-wrap gap-2'>
                                    {languageFilter.map((language => (
                                        <li key={language.id} onClick={() => onLanguageFilterItemClicked(language.id)} className={`${language.filterApplied ? "border border-red-500 bg-red-500 text-white" : "text-red-500 border border-gray-300"} py-1 px-3 text-sm cursor-pointer select-none`}>{language.languageName}</li>
                                    )))}
                                </ul>
                            </div>
                        </Accordion>
                            ...
                            <div className=''>
                                <ul className='flex flex-wrap gap-2'>
                                    {genreFilter.map((genre => (
                                        <li key={genre.id} onClick={() => { onGenreFilterItemClicked(genre.id) }} className={`${genre.filterApplied ? "border border-red-500 bg-red-500 text-white" : "text-red-500 border border-gray-300"} py-1 px-3 text-sm cursor-pointer select-none`}>{genre.genreName}</li>
                                    )))}
                                </ul>
                            </div>
                        </Accordion>
